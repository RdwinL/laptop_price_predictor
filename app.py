...
if mode == "Price Predictor":
    ...
    with col2:
        weight = st.selectbox("Weight (kg)", [1.0, 1.3, 1.8, 2.0, 2.5, 3.0, 4.0], key='weight_input')
    with col3:
        inches = st.selectbox("Screen Size (inches)", [11.0, 12.0, 13.0, 14.0, 15.0, 15.6, 16.0, 18.0], key='inches_input')
    with col4:
...
# Download Prediction Report as PDF
        def create_pdf_report():
            ...
            pdf.set_draw_color(214, 234, 248)
            ...
            pdf.cell(0, 8, f'{prediction:,.0f} Tsh', 0, 1)
            
            pdf.set_font('Arial', '', 11)
            pdf.cell(80, 8, 'Predicted Price (USD):', 0, 0)
            pdf.set_font('Arial', 'B', 11)
            pdf.cell(0, 8, f'${prediction/2500:.2f} USD', 0, 1)
            
            pdf.set_font('Arial', '', 11)
            pdf.cell(80, 8, 'Prediction Date:', 0, 0)
            pdf.set_font('Arial', 'B', 11)
            pdf.cell(0, 8, pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"), 0, 1)
            pdf.ln(5)
            ...
            pdf.set_font('Arial', '', 11)
            pdf.cell(80, 8, 'Features Used:', 0, 0)
            pdf.set_font('Arial', 'B', 11)
            pdf.cell(0, 8, str(len(feature_names)), 0, 1)
            ...
            pdf.cell(0, 5, f'Model: {model_name} | Accuracy: {model_data["test_r2_score"]:.1%}', 0, 1, 'C')
